...
class Api::UserController < Api::ApiController
  ...
  def sign_in
    ...
    #http://prueba.local:3000/user/sign_up?first_name="first_name_user"&last_name="last_name_user"&nick_name="nick_name_user"&email="email_user"&uid=72357278&token=12345678
    user = User.find_by_email(params[:request][:user][:email]) || User.save_user(params[:request][:user])
  ...
  private
    ...
    def get_user_keys
      ...
      user_keys =  user.user_key.blank? || user.user_key.save(user_auth)
